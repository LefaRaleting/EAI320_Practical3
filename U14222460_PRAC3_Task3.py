...
#Weight_HH= np.array([[-1.59091739,  0.71107276 , 0.36859646],[-0.41635835,  0.03107692,  0.03409049],[-1.56326268,  0.76438302 , 0.3889477 ]])
...
#Weight_HO= np.array([[ 0.60685563,  0.0353108 ,  0.69664273],[-0.92935851, -0.48687406,  0.91441461],[ 0.02415012, -0.00235646, -0.52089029]])
...
#    return np.exp(x)/np.exp(x).sum(axis=0)
    b=np.exp(x-np.max(x))/np.sum(np.exp(x-np.max(x)),axis=0)
    return b
#softmax derivative
def Softmaxderv(x):
    ...
    return (1-Softmax(x[0]))#np.diagflat(soft) - np.dot(soft, soft.T)
...
def Loss_functionDB(EOut,Out):
    L=0
    i=0
    for k in EOut:
        L+=Loss_function(outputEncoder[k[1]],Out[i])
        i+=1    
    return L
...
#timea=time.time()
#train(database,database,Weight_IH,Weight_HH,Weight_HO,gama,number_of_iterations,0.2)
#timeb=time.time()
...
if input == "":
    ...
    gama=0.2 #learning rate 0.1
    n=1# everytime it reaches this number it must train the network
    alpha=0.05#0.2
    
    previous="P"#np.random.choice(["R", "P", "S"]) 
    moves="" #this is to hold the moves
    matches=0
    temp=""
    history=[] #this is to hold the history of the moves
    rounds=0

else:
    if(matches<2):#because there isnt anything in the history for the first
        #two moves we must create a history 
        moves+=previous
        moves+=input
        previous="P"#np.random.choice(["R", "P", "S"])
        matches+=1
        ...
        history.append((moves,winningmove(input)))#winningmove(input)
        if(rounds==n):
            Weight_HO,Weight_HH,Weight_IH,Loss=train(history,history,Weight_IH,Weight_HH,Weight_HO,gama,number_of_iterations,alpha)
            rounds=0
            history=[]
            #matches=0
